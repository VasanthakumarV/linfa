...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let (train_data, train_targets) = (
        data.slice(s!(0..ntrain, ..)).to_owned(),
        targets.slice(s!(0..ntrain)).to_owned(),
    );
    let (valid_data, valid_targets) = (
        data.slice(s!(ntrain.., ..)).to_owned(),
        targets.slice(s!(ntrain..)).to_owned(),
    );

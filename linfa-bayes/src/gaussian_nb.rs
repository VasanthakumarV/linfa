...
impl<'a, A> Fit<'a, &Array2<A>, &Array1<usize>> for GaussianNbParams<A>
where
    A: Float + PartialEq + PartialOrd,
{
    ...
    /// # }
    /// ```
    fn fit(&self, dataset: &'a Dataset<&Array2<A>, &Array1<usize>>) -> Self::Object {
...
impl<A: Float> Predict<&Array2<A>, Result<Array1<usize>>> for GaussianNb<A> {
    /// Perform classification on incoming array
    fn predict(&self, x: &Array2<A>) -> Result<Array1<usize>> {
